...
/**
 * Configure Web Directory settings for this site.
 */
class WebdirConfigForm extends ConfigFormBase {
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    $config = $this->config('webspark_webdir.settings');
    $config->set('api', trim($form_state->getValue('api')));
    $config->set('api_version', trim($form_state->getValue('api_version')));
    $config->set('departments', trim($form_state->getValue('departments')));
    $config->set('people_department', trim($form_state->getValue('people_department')));
    $config->set('filtered_people_department', trim($form_state->getValue('filtered_people_department')));
    $config->set('people_search', trim($form_state->getValue('people_search')));
    $config->set('profile_affiliations', trim($form_state->getValue('profile_affiliations')));
    $config->set('people_data', trim($form_state->getValue('people_data')));
    $config->set('employee_types', trim($form_state->getValue('employee_types')));
    $config->set('expertise_areas', trim($form_state->getValue('expertise_areas')));
    $config->save();
