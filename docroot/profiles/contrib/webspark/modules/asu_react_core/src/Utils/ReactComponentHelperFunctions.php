...
class ReactComponentHelperFunctions {
   ...
   *
   */
  public function getButtonColor($options, $default) {
    $color = $default;
   ...
   *
   */
  public function processLink($raw_link) {
    if (isset($raw_link)) {
      if (preg_match('/^(https?|mailto|tel):/', $raw_link)) {
        return $raw_link;
      }
      // Drupal prepends internal links with 'internal:' so we need to strip it.
      elseif (str_starts_with($raw_link, 'internal:')) {
        $raw_link = preg_replace('/internal:/', '', $raw_link);
        $link = Url::fromUserInput($raw_link);
        return $link->toString() ?: $raw_link;
      }
      elseif (str_starts_with($raw_link, 'route:') || str_starts_with($raw_link, 'entity:') || str_starts_with($raw_link, 'base:')) {
