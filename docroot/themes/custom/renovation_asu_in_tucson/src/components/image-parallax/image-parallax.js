(function ($, Drupal) {
  Drupal.behaviors.imageParallax = {
    attach: function (context, settings) {
      ...
      $(once('paralaxContainer', '.parallax-container', context))
        .each(function () {
        ...
        $(once('paralaxContainerSectionCta', '.parallax-container.section-cta-buttons', context))
          .each(function () {
        ...
        $(once('paralaxContainerOverlay', '.parallax-container.overlay', context))
          .each(function () {
            // dataLayer elements focus event listener
            const elements = document.querySelectorAll('[data-ga-image-parallax-overlay]');
            elements.forEach((element) =>
              element.addEventListener('focus', () => {
                const args = {
                  section: element
                    .getAttribute('data-ga-image-parallax-overlay-section')
                    .toLowerCase(),
                  text: element.getAttribute('data-ga-image-parallax-overlay').toLowerCase(),
                };
                pushImageParallaxGAEvent(args);
              })
            );
