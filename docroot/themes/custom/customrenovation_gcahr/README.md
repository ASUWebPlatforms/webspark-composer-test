...
1. If you plan on installing this module via composer (`composer install asuwatts/customrenovation_gcahr:dev-main`), you will not be able to customize it for your site.
   ...
   2. Then you will need to remove it from your composer.json and composer.lock files: `composer remove asuwatts/customrenovation_gcahr`
   3. After that, you will need to make sure that the references to the Sass variables are updated. Most likely you will need to do a find/replace changing `../../../webspark-theme-renovation` to be `../../../composer/webspark-theme-renovation` in the "customrenovation_gcahr/src/sass/customrenovation_gcahr.style.scss" file (which is where you should make any Sass/css edits).
...
7. Navigate to the "web/themes/customrenovation_gcahr" subtheme folder in your terminal.
...
11. You can test if browsersync and the Sass variables are working correctly by making a change to the "customrenovation_gcahr/src/sass/customrenovation_gcahr.style.scss" file. An easy test is to change the color of links from maroon to blue.
```css
a {
  color: $blue;
}
