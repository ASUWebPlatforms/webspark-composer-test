...
(function($) {
  ...
  Drupal.behaviors.svgtoogle_conf = {
    attach: function() {
      var flag2 = true;
     /* if (flag) {
        $('#quiz_conf_top_conatiner').stop().delay(1500).animate({
                  'marginTop' : "-=23%",
                   'speed' : 'linear',
                  queue: 'my-animation'
                  });
     ...
     /* $('.persona_image').stop().delay(1000).animate({
         'opacity' : 0.4,
         queue: 'my-animation',
         'speed' : 'linear',
     ...
     /*call_resize();
      ...
      //hide explained text boxes on page load
      //$('.personality').siblings('.child2').addClass('child_hide');
    ...
    //Add extra css to each personna type div
    //console.log($('#superfan').attr('title') );
  ...
  //change data toggle to expandable text box  
  /*function call_resize(){
    ...
    if (w_width < 600) {
      $('.personality').hover(function(){
      //$('.personality').on('touchstart', function(e) {
        $(this).siblings('.child2').toggle();
