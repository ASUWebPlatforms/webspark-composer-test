...
class Video extends Component {
    ...
    setupVideoFunction(source, stream) {
        var self = this;
        console.log('My function is called',source, stream);
        console.log(self);
         if(!isMobile){
      ...
      highlightLink(stream) {
        ...
        componentDidMount() {
            ...
            this.videoResetInterval = setInterval(() => {
                console.log("Calling to reset with current stream ",this.state.stream);
                this.setupVideoFunction(this.props.source, this.state.stream);
            }, 240000);
    ...
    /* activeLinksFunction(childDivs){
        console.log(childDivs);
        for(var i=0; i<childDivs.length; i++ ){
            if(data['cam_mapping'][childDivs[i].innerHTML] === this.state.stream){
                // activate link
                childDivs[i].style.color = '#8c1d40';
            }
        }
                        ...
                        Object.keys(this.props.camOptions[heading]).map((key, i) => (
                            <div key={i}>
                                <div style={inlineStyle}>
                                    <p className="text" onClick={e => this.changeLink(e)} style={optionStyle}>{this.props.camOptions[heading][key]}</p>
                                </div>
                                {   i !== (this.props.camOptions[heading].length - 1) && 
                        ...
                        Object.keys(this.props.camOptions[heading]).map((key, i) => (
                            <div key={i}>
                                <Link   style={{ textDecoration: 'none', color: 'black' }} 
                                        to={(link) => {
                                            link.pathname = ""
                                            link.pathname += "/" + heading + "/" + data['cam_mapping'][this.props.camOptions[heading][key]]
