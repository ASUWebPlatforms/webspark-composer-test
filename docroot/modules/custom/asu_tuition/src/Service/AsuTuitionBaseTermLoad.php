...
class AsuTuitionBaseTermLoad {
   ...
   * }
   */
  public function getBaseTermLoad($result) {
    ...
    if ($values->acad_career === 'UGRD') {
      ...
      if ($record) {
        ...
        $base_term['fall_honors'] = (intval($result->acad_years['fall_term']) > intval($record['end_term']) ? '9999' : $record['base_term']);
        $base_term['spring_honors'] = (intval($result->acad_years['spring_term']) > intval($record['end_term']) ? '9999' : $record['base_term']);
        $base_term['summer_honors'] = (intval($result->acad_years['summer_term']) > intval($record['end_term']) ? '9999' : $record['base_term']);
      }
