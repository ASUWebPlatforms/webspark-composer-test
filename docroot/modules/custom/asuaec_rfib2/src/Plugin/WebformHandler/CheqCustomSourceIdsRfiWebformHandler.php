 ...
 *
 * @WebformHandler(
 *   id = "cheq_customsourceids_rfib2_webform_handler",
 *   label = @Translation("Post to middleware B2 with Cheq values with custom source IDs"),
 *   category = @Translation("Form Handler"),
 *   description = @Translation("Send the submission to Middleware B2 with Cheq values using custom source IDs"),
 *   cardinality = \Drupal\webform\Plugin\WebformHandlerInterface::CARDINALITY_UNLIMITED,
 *   results = \Drupal\webform\Plugin\WebformHandlerInterface::RESULTS_PROCESSED,
 *   submission = \Drupal\webform\Plugin\WebformHandlerInterface::SUBMISSION_REQUIRED,
 * )
 */
class CheqCustomSourceIdsRfiWebformHandler extends WebformHandlerBase {
  /**
  ...
  public function postToMiddleware($webform_submission, $env, $update) {
    ...
    if (($update == TRUE && self::$x == 0) || ($update == FALSE && self::$x == 0)) {
      ...
      if ($env == 'prod') {
        ...
        // Override source ID by path when configured.
        $path = \Drupal::service('path.current')->getPath();
        $current_path = \Drupal::service('path_alias.manager')->getAliasByPath($path);
        $path_mappings_raw = $config_data->get('ground_sourceid_prod_by_path');

        if (!empty($path_mappings_raw)) {
          $lines = preg_split('/\r\n|\r|\n/', $path_mappings_raw);
          ...
          foreach ($lines as $line) {
            ...
            [$mapped_path, $mapped_sourceid] = array_map('trim', explode('|', $line, 2));

            if ($mapped_path !== '' && $mapped_sourceid !== '') {
              // \Drupal::logger('cstest')->notice("mapped_path:<pre>" . htmlspecialchars($mapped_path) . "</pre>");
              // \Drupal::logger('cstest')->notice("current_path:<pre>" . htmlspecialchars($current_path) . "</pre>");

              if (rtrim($mapped_path, '/') === rtrim($current_path, '/')) {
                $sourceid = $mapped_sourceid;
                \Drupal::logger('cstest')->notice("source id1:<pre>" . htmlspecialchars($sourceid) . "</pre>");
                break;
              }
            }
          }
      ...
      // \Drupal::logger('cstest')->notice("source id2:<pre>" . htmlspecialchars($sourceid) . "</pre>");
      ...
      if ($country_from_phone_gadget !== '') {
        ...
        if ($phone !== '' && class_exists('\libphonenumber\PhoneNumberUtil')) {
          try {
            ...
            // \Drupal::logger('cstest')->notice('phone: @phone', [
            // '@phone' => $phone,
            // ]);
            ...
            // \Drupal::logger('cstest')->notice('number_proto: @number_proto', [
            // '@number_proto' => print_r($number_proto, TRUE),
            // ]);

            $region_code = $phone_util->getRegionCodeForNumber($number_proto);
            // \Drupal::logger('cstest')->notice('region_code: @region_code', [
