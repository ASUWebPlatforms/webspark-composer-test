(function ($, Drupal, drupalSettings) {
  Drupal.behaviors.rfi = {
    attach: function (context, settings) {
      console.log( "NOT USED: Moved to Asset Injector" ); // Moved JS to Asset Injector on 10/18/2025
      ...
      function grabURLparams() {
        ...
        if($('input[name="plan"]').val() != undefined) { // It becomes undefined in Step 2 and 3.
          ...
          if($('input[name="plan"]').val() != '[current-page:query:plan]') {
            ...
            // Grad
            //if(strFirstTwo == "GR") { // Changed on 10/8/2025
            if(strFirstTwo === "GR" || prog === 'MDMD')   {
              $('input[name="grad_ugrad"]').val('GRAD');
              $('select[name="student_type_options_default"]').val('Readmission'); // Added on 6/21/2024.
      ...
      function populateSelectList(jsonUrl, appendToElement, notListed = false,
          ...
          //clearSelectList(appendToElement, textFieldIdtoBeClearedArray, selectOptionIdtoBeClearedArray, textFieldNametoBeClearedArray); // Moved to inside ajax code below at line 151. (6/22/2024)

          // Change mouse pointer to wait
          $(document).ajaxStart(function() {
            $(appendToElement).css({'cursor' : 'wait'});
          }).ajaxStop(function() {
            $(appendToElement).css({'cursor' : 'default'});
          });
      ...
      function populateTermDropdownList() {
        // Clear message for unavailable degree
        $("#EntryTerm").remove();
        ...
        if (selectedval_campus_options == 'GROUND' || selectedval_campus_options == 'NOPREF') {
          if (selectedval_student_type_options_default == 'Readmission') {
            ...
            if(plancode != '') {
              ...
              if (drupalSettings.asuaec_rfib2.wsdirect == 'wsdirect') {
                jsonUrl = origin + '/admin/asuaec_rfib2/json/wsdirect/term/grad/' + plancode;
              } else {
                jsonUrl = origin + '/admin/asuaec_rfib2/json/term/grad/' + plancode;
              }
      ...
      if(cameFromDegreeSearch == true) {
        ...
        // Commented out on 6/21/2024. The same things were already done in grabURLparams().
        // // 7/6/2023
      ...
      /**
       * Clear Student type dropdown - Added on 5/21/2024
       */
      function clearSelection() {
        $('#edit-student-type-options-default').val('0');
        $('#edit-student-type-options-default').trigger('change');
      }
      ...
      // Ground Ugrad
      //$('select[name="area_of_interest_ugrad"]', context).once('rfi').on('change', function() {
      $(once('rfi', 'select[name="area_of_interest_ugrad"]')).on('change', function() { // D10 change
        ...
        if(cameFromDegreeSearch != true) { // If didn't come from Degree Search
          ...
          if (!(selectedValue == '' || selectedValue == '0' || selectedValue == null)) {
            let jsonUrl = '';
            if(drupalSettings.asuaec_rfib2.wsdirect == 'wsdirect') {
              jsonUrl = origin + "/admin/asuaec_rfib2/json/wsdirect/degrees/ground/ugrad/" + selectedValue;
            } else {
              jsonUrl = origin + "/admin/asuaec_rfib2/json/degrees/ground/ugrad/" + selectedValue;
            }
      ...
      // Ground Grad
      //$('select[name="area_of_interest_grad"]', context).once('rfi').on('change', function() {
      $(once('rfi', 'select[name="area_of_interest_grad"]')).on('change', function() { // D10 change
        ...
        if(cameFromDegreeSearch != true) { // If didn't come from Degree Search
          ...
          if (!(selectedValue == '' || selectedValue == '0' || selectedValue == null)) {
            let jsonUrl = '';
            if(drupalSettings.asuaec_rfib2.wsdirect == 'wsdirect') {
              jsonUrl = origin + "/admin/asuaec_rfib2/json/wsdirect/degrees/ground/grad/" + selectedValue;
            } else {
              jsonUrl = origin + "/admin/asuaec_rfib2/json/degrees/ground/grad/" + selectedValue;
            }
            let appendToElement = "#dynamic-program-interest";
      ...
      // Online Ugrad
      //$('select[name="area_of_interest_ugrad_online"]', context).once('rfi').on('change', function() {
      $(once('rfi', 'select[name="area_of_interest_ugrad_online"]')).on('change', function() { // D10 change
        ...
        if(cameFromDegreeSearch != true) { // If didn't come from Degree Search
          ...
          if (!(selectedValue == '' || selectedValue == '0' || selectedValue == null)) {
            if(drupalSettings.asuaec_rfib2.wsdirect == 'wsdirect') {
              jsonUrl = origin + "/admin/asuaec_rfib2/json/wsdirect/degrees/online/ugrad/" + selectedValue;
            } else {
              jsonUrl = origin + "/admin/asuaec_rfib2/json/degrees/online/ugrad/" + selectedValue;
            }
            let appendToElement = "#dynamic-program-interest";
            populateSelectList(jsonUrl, appendToElement, false, ['input[name="program_of_interest_text"]'], [], []);
          }
      ...
      // Online Grad
      //$('select[name="area_of_interest_grad_online"]', context).once('rfi').on('change', function() {
      $(once('rfi', 'select[name="area_of_interest_grad_online"]')).on('change', function() { // D10 change
        ...
        if(cameFromDegreeSearch != true) { // If didn't come from Degree Search
          ...
          if (!(selectedValue == '' || selectedValue == '0' || selectedValue == null)) {
            let jsonUrl = '';
            if(drupalSettings.asuaec_rfib2.wsdirect == 'wsdirect') {
              jsonUrl = origin + "/admin/asuaec_rfib2/json/wsdirect/degrees/online/grad/" + selectedValue;
            } else {
              jsonUrl = origin + "/admin/asuaec_rfib2/json/degrees/online/grad/" + selectedValue;
            }
            let appendToElement = "#dynamic-program-interest";
            populateSelectList(jsonUrl, appendToElement, false, ['input[name="program_of_interest_text"]'], [], []);
          }
      ...
      // Program of interest
      //('#dynamic-program-interest', context).once('rfi').on('change', function() {
      $(once('rfi', '#dynamic-program-interest')).on('change', function() { // D10 change
        // Write value to the original text field.
      ...
      // Grad Entry term
      //$('#dynamic-term', context).once('rfi').on('change', function() {
      $(once('rfi', '#dynamic-term')).on('change', function() { // D10 change
        // Write value to the original text field.
        $('input[name=entry_term_text]').val(this.value);
        // Cache selection in sessionStorage
        sessionStorage.setItem("selectedval_entry_term_text", this.value);
      ...
      // Default Entry term
      //$('select[name=entry_term]', context).once('rfi').on('change', function() {
      $(once('rfi', 'select[name=entry_term]')).on('change', function() { // D10 change
        // Write value to the original text field.
        $('input[name=entry_term_text]').val(this.value);
        // Cache selection in sessionStorage
        sessionStorage.setItem("selectedval_entry_term_text", this.value);
      ...
      // GROUND/ONLNE/NOPREF
      //$('select[name=campus_options]', context).once('rfi').on('change', function() {
      $(once('rfi', 'select[name=campus_options]')).on('change', function() { // D10 change
        // Cache selection in sessionStorage
      ...
      $(once('rfi', 'select[name=student_type_options_default]')).on('change', function() { // D10 change
        // Cache selection in sessionStorage
        sessionStorage.setItem("selectedval_student_type_options_default", this.value);
        if(this.value == 'Readmission') {
            $('input[name="grad_ugrad"]').val('GRAD');
            //populateTermDropdownList(); // Added on 7/6/2023. Commented out because it was displaying textarea#EntryTerm when it shouldn't be there on 6/22/2024.
        } else {
            $('input[name="grad_ugrad"]').val('UGRAD');
        }
        evaluateRequiredFields();
      ...
      // Consent
      //$('input#edit-gdpr-consent-1').once('rfi').on('change', function() {
      $(once('rfi', 'input#edit-gdpr-consent-1')).on('change', function() { // D10 change
        // Remove error class
        $(this).removeClass('error');
      });
      //$('input#edit-gdpr-consent-online-1').once('rfi').on('change', function() {
      $(once('rfi', 'input#edit-gdpr-consent-online-1')).on('change', function() { // D10 change
        // Remove error class
        $(this).removeClass('error');
      });
      ...
      //$('input[name=postal_code]', context).once('rfi').on('change', function() {
      $(once('rfi', 'input[name=postal_code]')).on('change', function() { // D10 change
          // Cache selection in sessionStorage
      ...
      //$('.form-item-campus-options > label').once('rfi').prepend(svgCode);
      $(once('rfi', '.form-item-campus-options > label')).prepend(svgCode); // D10 change
      //$('.form-item-student-type-options-default > label').once('rfi').prepend(svgCode);
      $(once('rfi', '.form-item-student-type-options-default > label')).prepend(svgCode); // D10 change
      //$('.form-item-area-of-interest-empty > label').once('rfi').prepend(svgCode);
      $(once('rfi', '.form-item-area-of-interest-empty > label')).prepend(svgCode); // D10 change
      if($('.js-form-item-program-of-interest-text > label > svg').length == 0) {
        //$('.form-item-program-of-interest-text > label').once('rfi').prepend(svgCode);
        $(once('rfi', '.form-item-program-of-interest-text > label')).prepend(svgCode); // D10 change
      }
      //$('.form-item-entry-term > label').once('rfi').prepend(svgCode);
      $(once('rfi', '.form-item-entry-term > label')).prepend(svgCode); // D10 change
      ...
      $('input[name="phone"]').keyup(function(){
        ...
        $(this).removeClass('is-invalid');
        // If Submit button is disabled, enable it.
        var $submitButton = $('#edit-submit');
        if ($submitButton.hasClass('disabled')) {
          $submitButton
            .removeClass('disabled')      // Remove the disabled class
            .prop('disabled', false)      // Enable the button
            .val('Submit');               // Change the button text back to "Submit"
