(function ($, Drupal, drupalSettings) {
 ...
 //           { name: "Shared File Services", matches: [1,2,4,5,6,7] }
    ...
    $.Mustache.add("question-checkbox-template",
        ...
        {{#description}}\
            <a class='control-indicator' data-bs-toggle='collapse' href='#explanation-{{id}}' role='button' aria-controls='explanation-{{id}}' aria-expanded='false'>\
                <span class='fa fa-info-circle' aria-hidden='true'></span>\
                <span class='sr-only'>More info about {{question}}</span>\
            </a>\
            </div>\
            <div class='explanation collapse' id='explanation-{{id}}'>\
                <p>{{{description}}}</p>\
            </div>\
        {{/description}}\
    ...
    $.Mustache.add("services-template",
        "{{#services}}\
        <div class='service-panel {{#contingent}} contingent {{/contingent}}' id='service-{{id}}' service='{{id}}'>\
            <label>\
    ...
    $.getJSON( "/rest/finder_settings", function( response ) {
        ...
        ASU_Resp = response.asu_responsibilities;
        ...
        $('#pageservicedetail').html(response.service_detail.replace(/(?:\r\n|\r|\n)/g, '<br />'));
        ...
        $('#pagefooterdisclaimers').html(response.footer_disclaimers.replace(/(?:\r\n|\r|\n)/g, '<br />'));;
    });
    ...
    $.getJSON( "/rest/facettree", function( response ) {
        ...
        for (i=0;i<readfacets.length;i++) {
            ...
            question.contingent = readfacets[i].contingent;
            question.choices = [];
...
// instead of labels, we have field_data[field]["label"]
        ...
        // every other row
        for (var i=0;i<(sortedfields.length + 1);i++) {
            if (i == (sortedfields.length)){
                //this is the last loop pop in the ASU responsibilities markup
                chart = chart + "<tr>";
                chart = chart + "<th scope='row'>"+'ASU Responsibilities';
                chart = chart +"</th>"; // row title
                for (var j=0;j<servicelist.length;j++){
                    console.log(ASU_Resp);
                    chart = chart + "<td class='service service-"+servicelist[j].id+"' data-label='asu_responsibilities'>"+ASU_Resp+"</td>";
                }
                chart = chart + "</tr>";
            }
            else {
                ...
                if (field){
                    ...
                    chart = chart +"</th>"; // row title
                        for (var j=0;j<servicelist.length;j++) {
                            var $field_value = servicelist[j].field_data[field].value;
                            if (!$field_value) {
                                $field_value = "N/A"
                            }
                            chart = chart + "<td class='service service-"+servicelist[j].id+"' data-label='"+servicelist[0].field_data[field].label+"'>"+$field_value+"</td>";
                        }
                    chart = chart + "</tr>";
                }
            }
