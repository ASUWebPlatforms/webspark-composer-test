...
use Drupal\asu_quikpay\Plugin\Commerce\PaymentGateway\Quikpay;
...
class QuikpayRedirectForm extends BasePaymentOffsiteForm {
  ...
  public function buildConfigurationForm(array $form, FormStateInterface $form_state) {
    ...
    // Per-site kill switch: when the new flow is disabled, send exactly the
    // request the develop version sent.
    if (empty($configuration['enable_rtpn_flow'])) {
      return $this->buildLegacyForm($form, $form_state, $payment, $configuration);
    }

    return $this->buildNewForm($form, $form_state, $payment, $configuration);
  }

  /**
   * Builds the outgoing request for the new (RTPN-capable) flow.
   */
  protected function buildNewForm(array $form, FormStateInterface $form_state, $payment, array $configuration) {
    ...
    if ($redirect_method == 'url') {
      ...
      // Single source of truth shared with the validation side, so the
      // requested parameter list and the hash validation can never desync.
      $param['redirectUrlParameters'] = implode(',', Quikpay::REDIRECT_RETURN_PARAMS);
  ...
  /**
   * Builds the outgoing request exactly as the develop version did.
   *
   * @deprecated Remove once the new flow is fully rolled out to all sites.
   */
  protected function buildLegacyForm(array $form, FormStateInterface $form_state, $payment, array $configuration) {
    $base_url = \Drupal::request()->getSchemeAndHttpHost();
