...
class NewsletterBreadcrumbBuilder implements BreadcrumbBuilderInterface
{
    ...
    public function applies(RouteMatchInterface $attributes)
    {
        ...
        if (!empty($parameters['node'])) {
            return $parameters['node']->getType() == 'newsletter';
        }
    }
    ...
    public function build(RouteMatchInterface $route_match)
    {
        ...
        if ($landing_page_field[0]['value'] != 1) {
            $query = \Drupal::entityQuery('node')
                ...
                ->condition('type', 'newsletter'); //content type
            $nids = $query->execute();
            foreach ($nids as $nid) {
                ...
                if ($landing_page_val[0]['value'] == 1) {
                    $landing_page = $nid;
                    $landing_page_title = $node->getTitle();
                    break;
                }
            }
            // $nid = $route_match->getParameter('node')->id();
            $breadcrumb->addLink(Link::createFromRoute($landing_page_title, 'entity.node.canonical', ['node' => $landing_page]));
        }
        return $breadcrumb;
    }
}
