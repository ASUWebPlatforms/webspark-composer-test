 ...
 *
 * @WebformHandler(
 *   id = "rfi_webform_handler_azpublicemployee",
 *   label = @Translation("Post to middleware - AZ Public Employee Scholarship"),
 *   category = @Translation("Form Handler"),
 *   description = @Translation("Send the submission to Middleware - AZ Public Employee Scholarship"),
 *   cardinality = \Drupal\webform\Plugin\WebformHandlerInterface::CARDINALITY_UNLIMITED,
 *   results = \Drupal\webform\Plugin\WebformHandlerInterface::RESULTS_PROCESSED,
 *   submission = \Drupal\webform\Plugin\WebformHandlerInterface::SUBMISSION_REQUIRED,
 * )
 **/
class AZPublicEmployeeRfiWebformHandler extends WebformHandlerBase {
  public static $x = 0; // Prevent posting twice. Added on 5/27/2022.

  /**
   * Added for AZ Scholarship.
   */
  protected $subClassOnlineProd;
  protected $subClassOnlineDev;
  protected $municipality; // Added on 4/22/2024.

  public function getSubClassOnlineProd() {
    return $this->subClassOnlineProd;
  }
  public function getSubClassOnlineDev() {
    return $this->subClassOnlineDev;
  }
  public function setSubClassOnlineProd($subClassOnline) {
    $this->subClassOnlineProd = $subClassOnline;
  }
  public function setSubClassOnlineDev($subClassOnline) {
    $this->subClassOnlineDev = $subClassOnline;
  }
  // Added on 4/22/2024
...
EOD;
  ...
  public function validateForm(array &$form, FormStateInterface $form_state, WebformSubmissionInterface $webform_submission) {
    ...
    if($the_submit_handler[0] == "::submit") {
      ...
      //------- Added for AZ Scholarship --------//
      // Grab City nid
      $citynid = isset($values['citynid']) ? $values['citynid'] : '104';

      // Load the node and get all info and save it in Class member variables.
      $city_node = Node::load($citynid);
//              $this->setSourceIdOnCampusProd($city_node->get('field_oncampus_sourceid')[0]->getValue());
//              $this->setSourceIdOnCampusDev($city_node->get('field_oncampus_sourceid_dev')[0]->getValue());
      $this->setMunicipality($city_node->get('field_city_display_name')[0]->getValue()); // Added on 4/22/2024

      // 12/15/2023 Testing
      if($env == 'prod'){
        $subclass_temp = $city_node->get('field_online_subclass')[0]->getValue()['value'];

      } else {
        $subclass_temp = $city_node->get('field_online_subclass')[0]->getValue();
      }
      $this->setSubClassOnlineProd($subclass_temp);


      // 12/15/2023 Testing
      if($env == 'prod'){
        $subclass_dev_temp = $city_node->get('field_online_subclass_dev')[0]->getValue()['value'];

      } else {
        $subclass_dev_temp = $city_node->get('field_online_subclass_dev')[0]->getValue();
      }
      $this->setSubClassOnlineDev($subclass_dev_temp);
      //------- END OF Added for AZ Scholarship --------//
...
//            if ($campus_options == 'GROUND' && $student_type_options_default == 'Readmission' || $campus_options == 'NOPREF' && $student_type_options_default == 'Readmission')
      ...
      $rfipage_alias = ($rfipage_alias != null || $rfipage_alias != '') ? $rfipage_alias : 'arizona-public-employee-scholarship-municipality-page-new';
      ...
      $config_data = \Drupal::config('asuaec_rfi.customadmin_settings');
      $env = 'prod';
      if($domain == $config_data->get('proddomain')) {
        $env = 'prod';
        ...
        $sub_class = $this->getSubClassOnlineProd();
        ...
        $sub_class = $this->getSubClassOnlineDev()['value'];
      }
...
//            \Drupal::logger('asuaec_rfi')->notice("asuonline_enterpriseclientid:<pre>" . $values['asuonline_enterpriseclientid'] . "</pre>");
      ...
      //-----------------------------------------------------------------------------------------------
      // ASU Online post if ASU Onlin is selected -- Added on 3/29/2022

      if($campus_options == 'ONLNE') {
//                $lead_class = 'CORP';
//                $sub_class = 'AMZ';

//                \Drupal::logger('asuaec_rfi')->notice('ASU Online sub_class: ' . htmlspecialchars($sub_class));
...
//                \Drupal::logger('asuaec_rfi')->notice('ASU Online post info_online: <pre>' . print_r($info_online, true) . '</pre>');
      ...
      // Middleware post if ASU Online is NOT selected -- Added on 3/29/2022
        ...
        $municipality_name = $this->getMunicipality()['value'];
        $comments = isset($values['comments']) ? $values['comments'] : '';
        if($comments == '') {
          $comments_new = $municipality_name;
        } else {
          $comments_new = $municipality_name . '|' . $comments;
        }
...
//                    'Comments' => isset($values['comments']) ? $values['comments'] : '',
          ...
          'GdprConsent' => isset($values['gdpr_consent'][0]) ? $values['gdpr_consent'][0] : '',
...
//            dpm($submission_data, "submission data");
...
//                    dpm($info, "info");
        ...
        if (($info['http_code'] < 200) || ($info['http_code'] >= 300)) {
          ...
          if($env == 'prod') {
            ...
            $the_error = new TranslatableMarkup('We are very sorry, error occured while posting data. Please try again later.<pre>' . htmlspecialchars(print_r($response, true)) . '<br />Middleware post URL: ' . htmlspecialchars($post_url) . '</pre>', array());
          }
          $this->messenger()->addError($the_error);
          $form_state->setRebuild();
          ...
          if($env == 'dev') {
            $the_message = new TranslatableMarkup('Success: <pre>' . htmlspecialchars(print_r($response, true)) . '<br />Middleware posted data:' . htmlspecialchars(print_r($submission_data, true)) . '<br />Middleware post URL: ' . htmlspecialchars($post_url) . '</pre>', array());
            $this->messenger()->addMessage($the_message);
          }
...
//            dpm("It was ::next or ::prev");
      $storage = $form_state->getStorage();
      $current_page = $storage['current_page'];
//            ksm($current_page, "current page");
      ...
      if($current_page == 'step_1') {
        ...
        if($values['campus_options'] == "GROUND" || $values['campus_options'] == "NOPREF") {
        ...
        else if($values['campus_options'] == "ONLNE") {
          if($values['student_type_options_default'] == "First Time Freshman" || $values['student_type_options_default'] == "Transfer") {
          ...
          else if($values['student_type_options_default'] == "Readmission") {
        ...
        else if($values['campus_options'] == "GROUND" || $values['campus_options'] == "NOPREF") {
          if($values['student_type_options_default'] == "Readmission") {
            if($values['area_of_interest_grad'] != "0" ) {
              if($values['program_of_interest_text'] == '' || $values['program_of_interest_text'] == '0') {
                $form_state->setErrorByName('program_of_interest_text', $this->t('Please select program of interest.'));
      ...
      // Step 2
      else if ($current_page == 'step_2') {
//              \Drupal::logger('asuaec_rfi')->notice('Step2!!!: ' . $values['ground_online']);
          // Entry term for Ground grad
        if(isset($values['entry_term_text'])) {
          if(($values['campus_options'] == "GROUND" && $values['student_type_options_default'] == "Readmission") || ($values['grad_ugrad'] == "GRAD" && $values['came_from_degree_search'] == "true") ) { // Changed on 5/10/2022.
