 ...
 *
 * @WebformHandler(
 *   id = "rfi_webform_handler",
 *   label = @Translation("Post to middleware"),
 *   category = @Translation("Form Handler"),
 *   description = @Translation("Send the submission to Middleware"),
    ...
    public function postSave(WebformSubmissionInterface $webform_submission, $update = TRUE) {
        ...
        if($campus_option != 'ONLNE') {
            ...
            if ($result['result'] !== true) {
                ...
                \Drupal::logger('asuaec_rfi')->notice('Main RFI - Email error: Confirmation email was not sent. - Email address:' . $to);
            } else {
                if($env == "dev") {
...
//        \Drupal::logger('asuaec_rfi')->notice("postSave - details array: " . print_r($degree_data_array, true));
        ...
        if(($campus_option == 'GROUND' && $grad_ugrad == 'GRAD') || ($campus_option == 'NOPREF' && $grad_ugrad == 'GRAD')) {
            $email_banner_image = t( $domain . '/sites/default/files/2025-04/grad_confirmation_600px.jpg');
...
//            }
        ...
        if(($campus_option == 'GROUND' && $grad_ugrad == 'UGRAD') || ($campus_option == 'NOPREF' && $grad_ugrad == 'UGRAD')) {
            ...
            $email_banner_image = t( $domain . '/sites/default/files/2025-04/email_txtphb_goldisgoingtolookgoodonyou.jpg');
  ...
  <tr>
    <td class='mobile-bump' style='border-collapse: collapse; text-align: left; font-size:16px; color:#000000; font-family: Arial, sans-serif; padding: 24px 24px 24px 24px; line-height: 24px; font-weight: 300;'>
      <a name='ASU Logo Link' href='https://asu.edu' target='_blank'>
        <img src='{$domain}/sites/default/files/2025-04/asu_logo_230px.png' width='230' height='auto' alt='Arizona State University' style='border-collapse: collapse; text-align: left; font-size:22px; color:#ffc627; font-family: Arial, sans-serif; padding: 0px; line-height: 24px; font-weight: bold;'>
      </a>
    </td>
  </tr>
</table></td></tr></table>
...
//            if ($campus_options == 'GROUND' && $student_type_options_default == 'Readmission' || $campus_options == 'NOPREF' && $student_type_options_default == 'Readmission')
            ...
            $rfipage_alias = ($rfipage_alias != null || $rfipage_alias != '') ? $rfipage_alias : 'future-request';
...
//                $post_url = 'https://crm-enterprise-rfi-forms-submit-handler-prod.apps.asu.edu/'; //<--- Old posting URL
            ...
            } else if ($domain == 'https://csdev45-admission-asu.ws.asu.edu') { // This is for MJ's testing at csdev45. Added on 5/24/2024.
                $env = 'dev';
                $sourceid = $config_data->get('ground_sourceid_dev');
                $post_url = 'https://crm-request-form-submission-router-dev.apps.asu.edu/v1/event/rfi'; //<--- Changed on 3/18/2024.
            } else {
                $env = 'dev';
                $sourceid = $config_data->get('ground_sourceid_dev');
